...
// Set when there are one or more tasks waiting to acquire the lock.
const HAS_WAITERS: usize = 1 << 1;
// Set when a task has been woken up from the wait queue. Cleared when that task either acquires
// the lock or adds itself back into the wait queue. Used to prevent unnecessary wake ups when a
// task has been removed from the wait queue but has not gotten CPU time yet.
...
// Set when a task that wants an exclusive lock adds itself to the wait queue. New tasks
// attempting to acquire a shared lock will be preventing from getting it when this bit is set.
// However, this bit is ignored once a task has gone through the wait queue at least once.
const WRITER_WAITING: usize = 1 << 4;
// Set when a task has gone through the wait queue many times but has failed to acquire the lock
// every time it is woken up. When this bit is set, all other tasks are prevented from acquiring
// the lock until the task that set the `LONG_WAIT` bit has acquired the lock.
const LONG_WAIT: usize = 1 << 5;
// The bit that is added to the mutex state in order to acquire a shared lock. Since more than one
// task can acquire a shared lock, we cannot use a single bit. Instead we use all the remaining
// bits in the state to track the number of tasks that have acquired a shared lock.
const READ_LOCK: usize = 1 << 8;
// Mask used for checking if any tasks currently hold a shared lock.
...
// high so that we don't lose the benefit of having running tasks unless it is absolutely
// necessary.
const LONG_WAIT_THRESHOLD: usize = 19;

// Common methods between shared and exclusive locks.
trait Kind {
    // The bits that must be zero for the task to acquire this kind of lock. If any of these bits
    // are not zero then the task will first spin and retry a few times before adding itself to
    // the wait queue.
    ...
    // The bits that should be set when a task adds itself to the wait queue while waiting to
    // acquire this kind of lock.
    fn set_when_waiting() -> usize;

    // The bits that should be cleared when a task acquires this kind of lock.
...
// A lock type for mutually exclusive read-write access to the data. Only one task can hold this
...
// set in the mutex state when the current task releases the spin lock protecting the waiter list.
//
...
// bit, which should be set when the task releases the spin lock.
//
...
impl RawMutex {
        ...
        loop {
            ...
            if (oldstate & zero_to_acquire) == 0 {
                    ...
                    .is_ok()
                {
                    ...
                    // Now that the task has waited once, we no longer care if there is a writer
    ...
    pub fn unlock(&self) {
        ...
        // Panic if we just tried to unlock a mutex that wasn't held by this task. This shouldn't
    ...
    fn unlock_slow(&self) {
        ...
        loop {
            ...
            if (oldstate & HAS_WAITERS) == 0 || (oldstate & DESIGNATED_WAKER) != 0 {
                ...
                // The spin lock is not held by another task. Try to acquire it. Also set the
                // `DESIGNATED_WAKER` bit since we are likely going to wake up one or more tasks.
                if self
                    .state
                    .compare_exchange_weak(
                        oldstate,
    ...
    fn cancel(&self, waiter: &Waiter, wake_next: bool) {
        ...
        if wake_list.is_empty() {
            // We're not waking any other tasks so clear the DESIGNATED_WAKER bit. In the worst
            // case this leads to an additional task being woken up but we risk a deadlock if we
            // don't clear it.
            clear |= DESIGNATED_WAKER;
        }
...
/// const N: usize = 10;
///
/// // Spawn a few tasks to increment a shared variable (non-atomically), and
/// // let the main task know once all increments are done.
/// //
...
///         // Our non-atomic increment is safe because we're the only task
...
impl<T: ?Sized> Mutex<T> {
    ...
    pub(crate) async fn read_lock_from_cv(&self) -> MutexReadGuard<'_, T> {
        // Tasks that have waited in the Condvar's waiter list don't have to care if there is a
...
mod test {
    ...
    use super::*;
    use crate::{block_on, Condvar, SpinLock};

...
///
/// Condition variables are used in combination with a `Mutex` when a task wants to wait for some
/// condition to become true. The condition must always be verified while holding the `Mutex` lock.
/// It is an error to use a `Condvar` with more than one `Mutex` while there are tasks waiting on
/// the `Condvar`.
///
/// # Examples
///
...
/// // let all tasks waiting on the Condvar know once the increments are done.
...
impl Condvar {
    ...
    /// Block the current task until this `Condvar` is notified by another task.
    ///
    /// This method will atomically unlock the `Mutex` held by `guard` and then block the current
    /// task. Any call to `notify_one` or `notify_all` after the `Mutex` is unlocked may wake up
    /// the task.
    ///
    /// To allow for more efficient scheduling, this call may return even when the programmer
    /// doesn't expect the task to be woken. Therefore, calls to `wait()` should be used inside a
    ...
    /// task until the `Condvar` is notified.
    ///
    /// # Panics
    ///
    ...
    fn add_waiter(&self, waiter: UnsafeRef<Waiter>, raw_mutex: &RawMutex) {
        ...
        // Release the spin lock. Use a direct store here because no other task can modify
    ...
    /// Notify at most one task currently waiting on the `Condvar`.
    ///
    /// If there is a task currently waiting on the `Condvar` it will be woken up from its call to
    /// `wait`.
    ///
    ...
    /// Notify all tasks currently waiting on the `Condvar`.
    ///
    /// All tasks currently waiting on the `Condvar` will be woken up from their call to `wait`.
    ///
    /// Unlike more traditional condition variable interfaces, this method requires a reference to
    /// the `Mutex` associated with this `Condvar`. This is because it is inherently racy to call
    /// `notify_one` or `notify_all` without first acquiring the `Mutex` lock. Additionally, taking
